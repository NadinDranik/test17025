...
async function main() {
  ...
  await exec(conn, `cp ${APP_DIR}/deploy/nginx-gost17025.conf /etc/nginx/sites-enabled/gost17025 && nginx -t`, 'nginx config');
  await exec(conn, 'systemctl reload nginx', 'nginx reload');
